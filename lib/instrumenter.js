...
(function (isNode) {
    ...
    var SYNTAX,
        ...
        guessMethodName = isNode ? require('./util/guess-method-name').guessMethodName : null,
        calculateFunctionNodeHash = isNode ? require('./file-signature.js').calculateFunctionNodeHash : function () { return ''; },
    ...
    function defaultWalker(node, walker) {
        ...
        if (node.type == SYNTAX.FunctionDeclaration.name || node.type == SYNTAX.FunctionExpression.name || node.type == SYNTAX.ArrowFunctionExpression.name) {
            node.funcId = this.currentState.func;
        }
        else {
            var parent = walker.parent();
            node.funcId = parent !== null ? parent.node.funcId : 0;
        }
            ...
            if (childNode && !childNode.skipWalk) {
                ...
                if (isArray(childNode)) {
                    ...
                    for (j = 0; j < childNode.length; j += 1) {
                        ...
                        pathElement.index = j;
                        if (childElement) {
                            if (childElement.type !== SYNTAX.FunctionDeclaration.name && childElement.type !== SYNTAX.FunctionExpression.name && childElement.type !== SYNTAX.ArrowFunctionExpression.name) {
                                childElement.funcId = node.funcId;
                            }
                    ...
                    if (childNode.type !== SYNTAX.FunctionDeclaration.name && childNode.type !== SYNTAX.FunctionExpression.name && childNode.type !== SYNTAX.ArrowFunctionExpression.name) {
                        childNode.funcId = node.funcId;
                    }
    ...
    function Instrumenter(options) {
        ...
        this.walker = new Walker({
            ArrowFunctionExpression: [this.arrowBlockConverter, this.coverFunction],
    ...
    function readSourceMaps(fullFilename){
        if (!fullFilename) return;
        var sourceMapsFilename = fullFilename+'.map';
        var contents,rawSourceMapJsonData,consumer;
        try {
    ...
    Instrumenter.prototype = {
         ...
         *  of this library.
         * @param {Object} metadata extra metadata about the file
         */
        instrumentSync: function (code, filename, metadata) {
            //console.log('Instrumenting ' + filename + " (metadata=" + JSON.stringify(metadata) + ")");
            var program;
            metadata = metadata || {};
            var isSourceMaps = metadata.sourceMaps==undefined ? true : !!metadata.sourceMaps;
            var sourceMaps = isSourceMaps && readSourceMaps(filename);
            if (typeof metadata.sourceMaps == "object") {
                sourceMaps = new sourceMap.SourceMapConsumer(metadata.sourceMaps);
            }
            ...
            return this.instrumentASTSync(program, filename, code, metadata, sourceMaps);
        },
         ...
         *  @param {Object} metadata extra metadata about the file
         *  @param {sourceMap} sourceMaps source maps, when available. Used to find the original source code location for methods.
         */
        instrumentASTSync: function (program, filename, originalCode, metadata, sourceMaps) {
            ...
            this.sourceMap = null;
            this.coverState = {
                path: (metadata && metadata.filename) || filename, //if metadata specified another filename, use that
                s: {},
                b: {},
                f: {},
                fnMap: {},
                ...
                branchMap: {},
                fnToBranch: {0: []},
                inputSourceMap: metadata.sourceMaps
            };
            if (metadata && metadata !== true) {
                this.coverState.metadata = metadata;
            }
         ...
         */
        instrument: function (code, filename, callback, metaData) {
            if (!callback && typeof filename === 'function') {
                callback = filename;
                filename = null;
            }
            try {
                callback(null, this.instrumentSync(code, filename, metaData));
            } catch (ex) {
                callback(ex);
            }
        },
        ...
        getPreamble: function (sourceCode, emitUseStrict) {
            ...
            var preambleHeader = this.opts.preambleHeader || [];
            code = preambleHeader.concat([
                "%STRICT%",
        ...
        functionName: function (node, line, location, guessResult) {
            this.currentState.func += 1;
            if (!this.coverState.fnToBranch.hasOwnProperty(this.currentState.func)) {
                this.coverState.fnToBranch[this.currentState.func] = [];
            }
            var id = this.currentState.func,
                ignoring = !!this.currentState.ignoring,
                name = guessResult.actualName,
                anonymous = guessResult.isAnonymous,
                clone = function (attr) {
                    var obj = location[attr] || /* istanbul ignore next */ {};
                    return { line: obj.line, column: obj.column };
                },
                hash = calculateFunctionNodeHash(node, true);                
                var start = clone('start'), end = clone('end');
                var originalPosition = this.getOriginalPosition(start);
            ...
            this.coverState.fnMap[id] = {
                ...
                originalFilename: originalPosition.filename,
                skip: ignoring || undefined
            };
            if (guessResult) {
                this.coverState.fnMap[id].guessedName = guessResult.name;
            }
            this.coverState.f[id] = 0;
            return id;
        },

        relPath: function(p1, p2) {
        try{
            ...
            if (p2[0] == '/') return p2; //Absolute path
            ...
            var p3 = parts1.concat(parts2);
            var finalPath = path.normalize(p3.join('/')).replace(/\\/g,'/');
            return finalPath;
        } catch(e) { 
            return p2; 
        }
        ...
        getOriginalPosition: function(start){
            var originalFilename;
            var originalStart;
             if (this.sourceMaps && start) {
                try {
                    //console.log('Checking original position for '+name+' at'+JSON.stringify(start));
                    var originalPosition = this.sourceMaps.originalPositionFor(start);//{ line: start.line, column: start[1] });
                    ...
                    if (originalPosition && originalPosition.source && originalPosition.line!==null && originalPosition.column!==null) {
                        ...
                        if (originalFilename.indexOf("webpack:///webpack/")===-1 &&
                            ...
                            originalFilename = this.relPath(this.coverState.path,originalFilename);
                            originalStart = {line:originalPosition.line,column:originalPosition.column};
                        }
                    }
                }
                catch(e) {
        ...
        coverFunction: function (node, walker) {
            ...
            this.maybeSkipNode(node, 'next');
            var parentNode = walker.ancestor(1).node;
            var guessResult = guessMethodName(node, parentNode);

            // end location for arrow functions that do not have a block statement body. example - (item) => item.toJSON()
            // https://github.com/istanbuljs/istanbul-lib-instrument/blob/9f8931e3c8821f1eb1b9dfa7febe5c1febd6eef4/src/visitor.js#L320
            // https://github.com/istanbuljs/istanbul-lib-instrument/blob/9f8931e3c8821f1eb1b9dfa7febe5c1febd6eef4/src/visitor.js#L198
        ...
        branchName: function (type, startLine, pathLocations, originalFilename) {
            var bName,
                paths = [],
                locations = [],
                i,
            ...
            this.coverState.branchMap[bName] = { line: startLine, type: type, locations: locations, originalFilename: originalFilename };
            return bName;
        },
        ...
        locationsForNodes: function (nodes) {
            ...
            for (i = 0; i < nodes.length; i += 1) {
                ...
                var originalPosition = this.getOriginalPosition(start);
                loc.originalStart = originalPosition.start;
                ret.push(loc);
            }
            return ret;
        },
        ...
        ifBranchInjector: function (node, walker) {
            ...
            var alreadyIgnoring = !!this.currentState.ignoring,
                ...
                bName = this.branchName('if', walker.startLineForNode(node), [
                    { originalStart: originalPosition.start, start: makeLoc(), end: makeLoc(), skip: ignoreThen || undefined },
                    { originalStart: originalPosition.start, start: makeLoc(), end: makeLoc(), skip: ignoreElse || undefined }
                ], originalPosition.filename),
                thenBody = node.consequent.body,
                elseBody = node.alternate.body,
                child;
            this.coverState.fnToBranch[node.funcId].push([this.currentState.branch, 0]);
            this.coverState.fnToBranch[node.funcId].push([this.currentState.branch, 1]);
        ...
        switchBranchInjector: function (node, walker) {
            ...
            var start = node && node.loc && node.loc.start;
            var originalPosition = this.getOriginalPosition(start);
            bName = this.branchName('switch', walker.startLineForNode(node), this.locationsForNodes(cases), originalPosition.filename);
            for (i = 0; i < cases.length; i += 1) {
                ...
                this.coverState.fnToBranch[node.funcId].push([this.currentState.branch, i]);
            }
        },
        ...
        conditionalBranchInjector: function (node, walker) {
            var start = node && node.loc && node.loc.start;
            var originalPosition = this.getOriginalPosition(start);
            var bName = this.branchName('cond-expr', walker.startLineForNode(node), this.locationsForNodes([node.consequent, node.alternate]), originalPosition.filename),
