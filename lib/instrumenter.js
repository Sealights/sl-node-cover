    ...
    Instrumenter.prototype = {
        ...
        coverFunction: function (node, walker) {
            ...
            if (!node.id) {
                ...
                if (anc) {
                    if (anc.property == "init" && anc.node && anc.node.type == "VariableDeclarator" && anc.node.id && anc.node.id.type == "Identifier") {
                        guessName = anc.node.id.name;
                    }
                    else if (anc.property == "value" && anc.node && anc.node.type == "Property" && anc.node.key && anc.node.key.type == "Identifier") {
