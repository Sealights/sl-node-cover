...
(function (isNode) {
    ...
    Instrumenter.prototype = {
        ...
        functionName: function (node, line, location, guessedName) {
            ...
            var id = this.currentState.func,
                ...
                var start = clone('start'), end = clone('end'), originalFilename, originalStart;
                if (this.sourceMaps) {
                    try {
            ...
            if (guessedName) {
                this.coverState.fnMap[id].guessedName = guessedName;
            }
        ...
        coverFunction: function (node, walker) {
            ...
            var guessName = "";
            if (!node.id) {
                var anc = walker.ancestor(1);
                if (anc) {
                    switch (anc.property) {
                        case "init":
                            if (anc.node && anc.node.type == "VariableDeclarator" && anc.node.id && anc.node.id.type == "Identifier") {
                                guessName = anc.node.id.name;
                            }
                            break;
                        case "value":
                            if (anc.node && anc.node.type == "Property" && anc.node.key && anc.node.key.type == "Identifier") {
                                guessName = anc.node.key.name;
                            }
                            break;
                        case "right":
                            if (anc.node && anc.node.type == "AssignmentExpression" && anc.node.left && anc.node.left.type == "MemberExpression" && anc.node.left.property && anc.node.left.property.type == "Identifier") {
                                guessName = anc.node.left.property.name;
                            }
                            else if (anc.node && anc.node.type == "AssignmentExpression" && anc.node.left && anc.node.left.type == "Identifier") {
                                    guessName = anc.node.left.name;
                                }
                            break;
                    }
                }
