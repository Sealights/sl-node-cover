...
function calculateFunctionNodeHash(node) {
    ...
    var methodHashObject = {type: "method", position: formatLocToArray(clonedNode.loc.start), hash: hash};
    return {
        hash: hash,
...
function calculateMethodBranches(node, branches) {
    switch (node.type) {
        ...
        case "LogicalExpression":
            if (node.left !== null) {
                branches.push(calculateSingleBranchHash(node.left, node.type));
            }
            if (node.right !== null) {
                branches.push(calculateSingleBranchHash(node.right, node.type));
            }
            break;
    }
}
...
function calculateSingleBranchHash(node, nodeType) {
    var clonedNode = astUtils.clone(node);
    var position = formatLocToArray(node.loc.start);
    traverse(clonedNode, {
        pre: function (node, parent, prop, idx) {
            if (node == clonedNode) return;
            if (node.type == "FunctionDeclaration" || node.type == "FunctionExpression") {
    ...
    var hash = md5(generated);
    return {
        fileBranchIndex: fileBranchIndex,
        type: nodeType,
        position: position,
        hash: hash
    };
        ...
        traverse(ast, {
            pre: function (node, parent, prop, idx) {
                if (node.type == "FunctionDeclaration" || node.type == "FunctionExpression") {
                    ...
                    var sigHash = calculateFunctionNodeSigHash(node);
                    var start = formatLocToArray(node.loc.start);
                    var end = formatLocToArray(node.loc.end);
