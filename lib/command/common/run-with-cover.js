...
var Module = require('module'),
    ...
    advMatcherFor = require('../../util/adv-file-matcher').matcherFor,
...
function usage(arg0, command) {
    ...
    console.error('\nUsage: ' + arg0 + ' ' + command + ' [<options>] <executable-js-file-or-command> [-- <arguments-to-jsfile>]\n\nOptions are:\n\n'
        + [
            ...
            formatOption('--scan-deps', 'scan for code in dependencies that have sealights.json (off by default)'),
            formatOption('--branchCoverage', 'collect branch coverage with sealights')
        ].join('\n\n') + '\n');
    console.error('\n');
}
...
function run(args, commandName, enableHooks, callback) {
    ...
    var template = {
        ...
        overrides = {
            ...
            instrumentation: {
                ...
                'branchCoverage': opts['branchCoverage']
            },
            reporting: {
                reports: opts.report,
                print: opts.print,
    ...
    if (enableHooks) {
        ...
        excludes.push(path.relative(process.cwd(), reportingDir + '/**').replace(/\\/g, '/'));
        advMatcherFor({
            ...
            function (err, matchFn) {
                ...
                var coverageVar = '$$cov_' + new Date().getTime() + '$$',
                    instrumenter = new Instrumenter({ coverageVariable: coverageVar, preserveComments: preserveComments, branchCoverage: config.instrumentation.config.branchCoverage }),
