    ...
    copyDir: function (from, to, ignoredRelFiles, callback) {
        ...
        filesFor({
            ...
            files.forEach(function (relFile) {
                if (filesLookup[relFile]) return; //already processed/instrumented earlier
                try {
                    ...
                    if (stat.isDirectory()) {
                        ...
                        dirsLookup[outputFile] = true;
                    } else if (stat.isFile()) {
                        fs.writeFileSync(outputFile, fs.readFileSync(inputFile));
                    }
                } catch (e) {
                    console.error("Error copying file " + inputFile + " to " + outputFile);
                }
            });
            return callback(err);
        });
        ...
        if (hasMemoryFile) {
            ...
            if (opts.copyAllFilesToOutput) {
                ...
                callback = function (err) {
                    //An error may be caused by unparseable file(s). We report those but want to continue the process
                    _this.copyDir(opts.sourceRoot, opts.outputPath, opts.files, origCallback);
                    return origCallback(err);
                }
            }

            try {
                processFiles(instrumenter, opts.sourceRoot, opts.outputPath, opts.files, ['.js'], callback, {
