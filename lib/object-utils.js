...
(function (isNode) {
    ...
    function runInstrumentation (opts, callback) {
        //opts should include:
        // files: list of files
        // outputPath: folder where to put files
        // instrumentationType: "browser"
        // sourceRoot: root folder
        if (!callback) throw new Error("callback was not specified");
        if (!opts) throw new Error("opts was not specified");
        if (!opts.files || !opts.files.length) return callback(null);
        if (!opts.outputPath) return callback(new Error("outputPath was not specified"));
        if (opts.instrumentationType!="browser") return callback(new Error("instrumentationType must be 'browser'"));
        if (!sourceRoot) return callback(new Error("sourceRoot was not specified"));

         try {
             var commandObject = Command.create('instrument');
 
             commandObject.run(["./","-o","./dist-instrumented","--variable",""], function(err){ 
                 callback(err);
             });
         } catch (ex) {
             callback(ex);
    ...
    var exportables = {
        ...
        runInstrumentation: runInstrumentation
    };
