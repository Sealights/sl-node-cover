...
(function (isNode) {
    ...
    function runInstrumentation (opts, callback) {
         try {
             ...
             commandObject.runWithSlInstrumentation(opts, callback);
             /*opts.additionalFilesToCopy.forEach(function(name) {
                try {
                    var inputFile = path.resolve(opts.sourceRoot, name);
                    var outputFile = path.resolve(opts.outputPath, name);
                    var dir = path.dirname(outputFile);
                    mkdirp.sync(dir);
                    fs.writeFileSync(outputFile, fs.readFileSync(inputFile));
                } catch(e) {
                    console.error('Error copying ' + inputFile + ' to ' + outputFile + ': ' + e);
                }
             });*/
         } catch (ex) {
             callback(ex);
