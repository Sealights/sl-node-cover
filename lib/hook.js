...
function transformFn(matcher, transformer, verbose) {
    ...
    return function (code, filename) {
        var isFilenameAString = typeof filename === 'string',
            matchResult = isFilenameAString ? matcher(path.resolve(filename)) : null,
            shouldHook = isFilenameAString && !!matchResult,
            transformed,
            changed = false;
        ...
        if (shouldHook) {
            ...
            try {
                transformed = transformer(code, filename, matchResult);
                changed = true;
            } catch (ex) {
